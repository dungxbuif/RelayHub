...
func (client *Client) ClaimOutbox(ctx context.Context, now, staleBefore time.Time, claimToken string, limit int) ([]store.OutboxMessage, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&message.ID, &message.EventID, &message.DeliveryID, &message.Subject, &message.Payload, &message.MessageID, &message.ClaimToken, &message.Attempts, &message.CreatedAt); err != nil {
			return nil, err
		}
