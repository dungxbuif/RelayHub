...
it("shows new credentials once and requires explicit secure-storage acknowledgement", async () => {
  ...
  const user = userEvent.setup(); render(<AppProviders><App /></AppProviders>); await user.type(screen.getByLabelText("Bootstrap Admin token"), "bootstrap"); await user.click(screen.getByRole("button", { name: "Sign in" })); await user.type(await screen.findByLabelText("Name"), "Orders"); await user.click(screen.getByRole("button", { name: "Save app" }));
