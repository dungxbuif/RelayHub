...
| --- | --- | --- | --- |
| `RH_DELIVERIES` | `rh.deliveries.*` | work queue | `168h` |
| `RH_CALLBACKS` | `rh.callbacks.*` | work queue | `168h` |
| `RH_DLQ` | `rh.dlq.*` | limits | `168h` |
...
`/healthz` reports only process liveness. `/readyz` returns 503 when any required
store, NATS connection or JetStream account/configuration check fails. Prometheus
