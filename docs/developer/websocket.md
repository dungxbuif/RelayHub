...
Implementation record: protocol and hub tests cover local subscriptions, HTTP/client authentication, notifier ordering, prefix validation, NATS cross-instance fan-out and namespace behavior. Function hooks use persisted claims and results. Remote functions have no durable offline queue; their separate [claim, timeout and reconnect rules](./functions.md) apply.
