...
The service owns validation and credential policy; HTTP handlers translate service errors into the JSON contract; Redis owns atomic persistence and credential-index rotation. Task 2 verification covers service and transport contracts, real Redis behavior, concurrent uniqueness, race detection, vet, docs parity, and the production Docker build.
