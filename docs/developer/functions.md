...
Inbound WebSocket messages and complete RPC envelopes retain the 64 KiB bound. HTTP bodies remain capped at 1 MiB; invocation validation additionally bounds the fully serialized rpc.invoke frame before publication. Outbound event notifications use the accepted event size plus envelope overhead. Registrations persist until owner deletion; delete removes the name index. All ephemeral records have a 24 hour TTL; there are no presence keys.
