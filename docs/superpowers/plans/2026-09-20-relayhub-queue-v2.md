...
**Architecture:** PostgreSQL remains durable truth for subscriptions and queue deliveries. Event publication fans out transactionally to every enabled target-app subscription. Pull uses `FOR UPDATE SKIP LOCKED`; random receipt hashes plus delivery generation fence ACK/retry/dead-letter/extension. Shared admission/rate policy is enforced under the subscription's durable lock, while long polling uses bounded retry/wakeup and never holds a database transaction open.
...
- Add starvation-bounded priority selection, absolute and recurring schedules with explicit timezone/frequency bounds.
- Add explicit batch DLQ replay/delete/export with append-only audit and safe bounds.
- Add success/failure result callbacks carrying metadata only; callback work and replay are generation fenced.
- Add subscription drain and policy-version inspection without global ordering or exactly-once claims.
...
The remaining recurring-schedule, result-callback, audit and starvation-control
work is accepted in the locked advanced plan; it is not silently dropped from
the platform scope.
